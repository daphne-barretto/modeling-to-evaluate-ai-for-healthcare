...
end-to-end on a single CPU laptop without a GPU. The Modal stages
(Steps 1–2) are only needed if you want to re-run inference from scratch.

---

## Figures and tables produced by each script

Every figure and table referenced in the manuscript is regenerated by
`scripts/refit_all.py`. The table below lists the specific module that
